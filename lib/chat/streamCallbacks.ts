...
type LoggedStep = {
  stepNumber?: number;
  text?: string;
  content?: Array<LoggedPart | undefined>;
  toolResults?: Array<LoggedPart | undefined>;
  tool_results?: Array<LoggedPart | undefined>;
  results?: Array<LoggedPart | undefined>;
};
...
export function logChatStep(step: LoggedStep) {
  const content = (step.content ?? []).filter((part): part is LoggedPart => Boolean(part));
  const toolCalls = content
    .filter(part => part.type === 'tool-call')
    .map(part => part.toolName);

  console.log('[chat] step', step.stepNumber ?? '?', '| text length:', step.text?.length ?? 0, '| tool calls:', toolCalls);
  ...
  const resultParts = content.filter(part => part.type === 'tool-result');
